...
Write-Host "& `"$Python`" scripts\test_wake_text.py `"小黄ang。`""
Write-Host ""
...
Write-Host "& `"$Python`" scripts\voice_overlay.py --device 0 --debug --enable-tts"
Write-Host ""
Write-Host "Voice overlay with DeepSeek single-turn reply and TTS enabled:"
Write-Host '$env:DEEPSEEK_API_KEY="your_key_here"'
Write-Host "& `"$Python`" scripts\voice_overlay.py --device 0 --debug --enable-llm --enable-tts"
Write-Host ""
Write-Host "Voice overlay with longer cooldown if TTS is captured by wake checks:"
Write-Host "& `"$Python`" scripts\voice_overlay.py --device 0 --debug --enable-tts --post-response-cooldown 8"
Write-Host ""

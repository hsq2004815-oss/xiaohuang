...
def main() -> int:
    ...
    worker = threading.Thread(target=_run_overlay_loop, args=(app, options, logger, args.debug, stop_event), daemon=True)
    worker.start()
    root.mainloop()
    stop_event.set()
...
def _run_overlay_loop(app: VoiceOverlayApp, options: WakeLoopOptions, logger, debug: bool, stop_event: threading.Event) -> None:
    while not stop_event.is_set():
        try:
            result = run_wake_loop_once(
                options,
                on_state_change=lambda state, payload=None: app.thread_safe_set_state(state, payload),

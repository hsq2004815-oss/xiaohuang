...
C3 upgrades Agent Handoff from a XiaoHuang-only prompt generator into a generic project task package generator. `AgentHandoffRequest` now separates the XiaoHuang project root from the target project path with `target_project_path`, `target_project_kind`, and `project_relation`. XiaoHuang still writes handoff files only under its own `runtime/agent_handoffs/`, but the generated prompt can tell Claude Code / Codex to work in an external project such as `E:\Projects\sample-project`.

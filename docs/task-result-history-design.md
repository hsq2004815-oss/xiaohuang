...
## 十八、B1.1 补充：路径隔离与读取 API

### 路径隔离

B1 初始实现使用模块级 `_history_path` 变量缓存存储路径，当一个控制面板进程先后操作不同 `project_root` 时，后续 `append_task_result` 可能写到错误的 root。

B1.1 修复方案（方案 A — 单缓存 + root 切换）：

- 新增 `_cache_project_root: Path | None` 模块级变量，记录当前缓存对应的 project_root。
- 新增 `_ensure_cache_for_root(project_root)` — 检测传入 root 与 `_cache_project_root` 是否一致，不一致时自动调用 `init_task_history(root)` 重新加载缓存。
- `append_task_result()` 和 `get_recent_task_results()` 始终基于传入的 `project_root` 计算文件路径并自动切换缓存。
- 删除 B1 中无效的 `pass` 分支逻辑。

### 读取 API

`ControlPanelWebApi.get_recent_task_history(payload)` — 读取最近任务历史。

- 默认 `limit=20`，最小 1，最大 50。
- 非数字/负数/超大值安全 clamp。
- 文件不存在时返回 `ok=True, items=[]`（不是错误）。
- 返回结构：`{ ok, data: { items: [...] }, message }`。
- API 响应不包含文件路径或目录结构。

---

## 十九、B2/B3 UI Layout Plan（后续 UI 布局规划）

### 1. 总体布局原则

```
1. Tasks 页面是任务历史的主入口。
2. Chat 页面右侧栏只做"最近任务"轻量入口，不承担完整历史管理。
3. Chat 消息区不塞大量历史列表，避免干扰对话主流程。
4. 任务详情采用右侧 detail panel 或展开卡片，不用弹窗。
5. 任务历史卡片只展示安全摘要，不展示原始日志全文。
6. 搜索/筛选以后再做，不在第一版 UI 中堆满按钮。
```
...
│ ┌────────────────────────┐ │ 标题：小黄健康检查                    │
│ │ 小黄健康检查   有警告    │ │ 状态：完成                           │
│ │ 总体状态：有警告...     │ │ 完成时间：2026-05-10 14:30:02        │
│ │ 2分钟前  health 5files │ │                                     │
│ └────────────────────────┘ │ 摘要：                               │
│ ┌────────────────────────┐ │ 总体状态：有警告。历史日志中发现...    │
│ │ 配置摘要       完成     │ │                                     │
│ │ 当前配置摘要：...       │ │ 安全详情：                           │
│ │ 5分钟前  config 0files │ │ 一、基础状态 — 6/6 正常             │
│ └────────────────────────┘ │ 二、配置状态                          │
│ ┌────────────────────────┐ │   - LLM: 已启用...                  │
│ │ 最近错误       完成     │ │                                     │
│ │ 读取最近日志中的错误...  │ │ tags: readonly, health              │
│ │ 8分钟前  logs   2files │ │ 读取文件: 0                         │
│ └────────────────────────┘ │                                     │
└────────────────────────────┴─────────────────────────────────────┘
```

B2 第一版可简化为：主区域 = 最近任务列表 + 右侧 = 选中任务详情面板。

### 3. Chat 右侧栏建议布局

Chat 页面已有 utility rail（`text-chat-sessions` 或右侧栏区域）。
...
```
1. 最多展示 5 条。
2. 只显示 title + status + time。
3. 点击可打开 Tasks 页面，或将安全详情带回 Chat（具体交互 B2 决定）。
4. 不在 Chat 右侧栏展示完整 safe_details_excerpt。
5. 不抢占 Chat 主区域空间。
```
...
```
第一行：title + status badge（完成/失败/有警告）
第二行：summary（最多 1-2 行，truncate）
第三行：completed_at + tags + read_files_count
```

状态 badge 语义：

```
任务状态 (task status)：完成 (completed) / 失败 (failed)
报告结果 (report signal)：正常 / 有警告 / 有错误 / 信息不足
```

两项独立显示，不混淆。例如：任务状态"完成" + 报告结果"有警告"。

### 5. 详情面板字段

选中任务后在右侧 detail panel 展示：

```
显示：title, task_type, completed_at, status, summary,
       safe_details_excerpt, tags, read_files_count
弱化：history_id（可复制，但默认灰色小字）
```

不展示：

```
原始 details 全文、完整日志、完整 traceback、文件路径列表、任何敏感信息
```

### 6. B2 / B3 分工

```
B2：后端读取 API（已在 B1.1 完成）+ 前端 Tasks 页面主列表（优先做这个）。
     先不做 Chat 右侧栏最近任务入口，保持 Chat 干净。

B3：Chat 右侧栏最近 5 条轻量入口 + Tasks 页面按标签筛选 + 搜索。
```

选择先做 Tasks 页面而不是 Chat 右侧栏的原因：
- Tasks 页面更适合承载完整历史
- Chat 保持干净专注对话
- Chat 右侧栏后续只做最近 5 条快捷入口

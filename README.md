# 小黄 V0.9 DeepSeek 单句对话原型

小黄是一个 Windows 桌面 AI 助手项目。当前 V0.9 在 V0.8 单句回复链路上增加可选 DeepSeek-V4-Flash 单句回复：唤醒后听一句话、转写、可调用 DeepSeek 生成一句短回复，并可用 TTS 播放。
...
- `DEEPSEEK_API_KEY`: 必填；不允许写入仓库或配置文件
...
如果启用 `--enable-llm` 但未配置 `DEEPSEEK_API_KEY`，控制台会提示：

```text
DeepSeek API key 未配置，已使用本地规则回复
```
...
V0.9 只做单句回复，不保存历史上下文；如果用户要求打开浏览器、发消息、写代码、下载资料等实际操作，小黄只能回复：
